...
func NewExtend(logger ticket.ExtendLogger, signer api_token.TicketSigner, tickets ticket.TicketRepository) Extend {
	return Extend{
		logger:  logger,
		signer:  signer,
		tickets: tickets,
...
func (action Extend) Extend(request request.Request, user user.User, oldTicket api_token.Ticket) (_ api_token.Ticket, _ time.Expires, err error) {
	...
	newTicket := api_token.NewTicket(token, oldTicket.Nonce())
