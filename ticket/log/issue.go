...
func (log Logger) Issue(request request.Request, user user.User, expires time.Expires, limit time.ExtendLimit, nonce ticket.Nonce) {
	log.logger.Info(issueEntry("Issue", request, user, expires, limit, &nonce, nil))
}

func issueEntry(event string, request request.Request, user user.User, expires time.Expires, limit time.ExtendLimit, nonce *ticket.Nonce, err error) log.Entry {
	return log.Entry{
