...
func (log Logger) Extend(request request.Request, user user.User, expires expiration.Expires, limit expiration.ExtendLimit) {
	log.logger.Info(extendEntry("Extend", request, user, &expires, &limit, nil))
}

func extendEntry(event string, request request.Request, user user.User, expires *expiration.Expires, limit *expiration.ExtendLimit, err error) log.Entry {
	return log.Entry{
