...
func NewJWTSerializer(key JWTKey) JWTSerializer {
...
func (serializer JWTSerializer) Parse(token string) (jwt.MapClaims, error) {
	var claims jwt.MapClaims
	jwtToken, err := jwt.ParseWithClaims(token, &claims, func(token *jwt.Token) (interface{}, error) {
		return serializer.key.verifyKey(), nil
	})
	if err != nil {
		return nil, err
	}
...
func (serializer JWTSerializer) Serialize(claims jwt.MapClaims) (string, error) {
	key := serializer.key
