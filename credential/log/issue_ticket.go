...
func (log Logger) TryToIssueTicket(request request.Request, user user.User, expires expiration.Expires) {
	log.logger.Debug(issueTicketEntry("TryToIssue", request, user, expires, nil))
}
func (log Logger) FailedToIssueTicket(request request.Request, user user.User, expires expiration.Expires, err error) {
	log.logger.Error(issueTicketEntry("FailedToIssue", request, user, expires, err))
}
func (log Logger) IssueTicket(request request.Request, user user.User, expires expiration.Expires) {
	log.logger.Info(issueTicketEntry("Issue", request, user, expires, nil))
}

func issueTicketEntry(event string, request request.Request, user user.User, expires expiration.Expires, err error) log.Entry {
	return log.Entry{
