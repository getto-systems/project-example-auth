...
func (log Logger) TryToIssueContentToken(request request.Request, user user.User, expires credential.Expires) {
	log.logger.Debug(issueContentTokenEntry("TryToIssue", request, user, expires, nil))
}
func (log Logger) FailedToIssueContentToken(request request.Request, user user.User, expires credential.Expires, err error) {
	log.logger.Error(issueContentTokenEntry("FailedToIssue", request, user, expires, err))
}
func (log Logger) IssueContentToken(request request.Request, user user.User, expires credential.Expires) {
	log.logger.Info(issueContentTokenEntry("Issue", request, user, expires, nil))
}

func issueContentTokenEntry(event string, request request.Request, user user.User, expires credential.Expires, err error) log.Entry {
	return log.Entry{
